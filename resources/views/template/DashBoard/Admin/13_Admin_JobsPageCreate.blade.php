@include('template.DashBoard.Admin.0_Admin_Top')
  ...
  <main id="main" class="main" style="min-height: 610px">
    ...
    <div class="border p-3">
      <form action="{{ route('JobsPageStoreAdmin') }}" method="post" enctype="multipart/form-data">
        @csrf
        ...
        <div class="form-group">
          <label>Jobs Page Title</label>
          <input type="text" name="title" value="{{ optional($jobsPageData)->title }}" class="form-control" style="width: 50%;">
        </div>
        
        <br>
        
        <div class="form-group">
          <label>Jobs Sub Title</label>
          <input type="text" name="sub_title" value="{{ optional($jobsPageData)->sub_title }}" class="form-control">
        </div>

        <br>

        <label>Jobs Banner Image</label>
        <div class="content" style="text-align: center; margin:10px 0px 10px 0px; padding: 10px 10px 10px 10px; background: rgb(253, 253, 253); min-height: 100px;">
          <img id="imagePreview" src="{{ url('upload/jobsBanner/' . optional($jobsPageData)->bg_img)  }}" style="display: block;" height="95vh" width="100%">
        </div>
